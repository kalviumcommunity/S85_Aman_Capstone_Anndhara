...
export function useCart() {
  ...
  useEffect(() => {
    ...
    const fetchCart = async () => {
      try {
        setLoading(true);
        const res = await axios.get('https://anndhara.onrender.com/cart/', {
  ...
  const addToCart = async (crop, quantity = 1, proposedPrice) => {
    if (!isBuyer) return;
    try {
      const res = await axios.post('https://anndhara.onrender.com/cart/add', {
        cropId: crop._id,
        quantity,
  ...
  const removeFromCart = async (cropId) => {
    if (!isBuyer) return;
    try {
      const res = await axios.delete(`https://anndhara.onrender.com/cart/remove/${cropId}`, {
        headers: { Authorization: `Bearer ${user.token}` }
      });
      setCart(res.data.cart.items || []);
    } catch (err) {
  ...
  const clearCart = async () => {
    if (!isBuyer) return;
    try {
      const res = await axios.delete('https://anndhara.onrender.com/cart/clear', {
        headers: { Authorization: `Bearer ${user.token}` }
      });
      setCart([]);
    } catch (err) {
