...
const Cart = () => {
  ...
  useEffect(() => {
    const fetchCart = async () => {
      setLoading(true);
      try {
        const res = await fetch("https://anndhara.onrender.com/cart/", {
  ...
  const handleUpdateItem = async (cropId, quantity, proposedPrice) => {
    try {
      const res = await fetch("https://anndhara.onrender.com/cart/add", {
        method: "POST",
        headers: {
  ...
  const handleRemoveItem = async (cropId) => {
    try {
      const res = await fetch(`https://anndhara.onrender.com/cart/remove/${cropId}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${user.token}` },
      });
      const data = await res.json();
